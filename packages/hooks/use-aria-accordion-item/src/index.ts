import {
  ...
  /**
   * @deprecated use useReactAriaAccordionItem instead from @simao234430/use-aria-accordion
   */
    ...
    const extendFocusSelection = useCallback(
      (toKey: Key) => {
        if (manager.selectionBehavior === "replace") {
          manager.extendSelection(toKey.toString());
        }
        manager.setFocusedKey(toKey.toString());
      },
      [manager],
    );
