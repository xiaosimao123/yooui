...
import type {ConfigTheme, ConfigThemes, DefaultThemeType, YooUIPluginConfig} from "./types";
  ...
  for (const [themeName, {extend, layout, colors}] of Object.entries(themes)) {
    ...
    for (const [colorName, colorValue] of Object.entries(flatColors)) {
      ...
      try {
        ...
        const YooUIColorVariable = `--${prefix}-${colorName}`;
        const YooUIOpacityVariable = `--${prefix}-${colorName}-opacity`;

        // set the css variable in "@layer utilities"
        resolved.utilities[cssSelector][YooUIColorVariable] = `${h} ${s}% ${l}%`;
        // if an alpha value was provided in the color definition, store it in a css variable
        if (typeof defaultAlphaValue === "number") {
          resolved.utilities[cssSelector][YooUIOpacityVariable] = defaultAlphaValue.toFixed(2);
        }
        // set the dynamic color in tailwind config theme.colors
        resolved.colors[colorName] = ({opacityVariable, opacityValue}) => {
          // if the opacity is set  with a slash (e.g. bg-primary/90), use the provided value
          if (!isNaN(Number(opacityValue))) {
            return `hsl(var(${YooUIColorVariable}) / ${opacityValue})`;
          }
          // if no opacityValue was provided (=it is not parsable to a number)
          // the YooUIOpacityVariable (opacity defined in the color definition rgb(0, 0, 0, 0.5)) should have the priority
          // over the tw class based opacity(e.g. "bg-opacity-90")
          // This is how tailwind behaves as for v3.2.4
          if (opacityVariable) {
            return `hsl(var(${YooUIColorVariable}) / var(${YooUIOpacityVariable}, var(${opacityVariable})))`;
          }

          return `hsl(var(${YooUIColorVariable}) / var(${YooUIOpacityVariable}, 1))`;
        };
      } catch (error: any) {
...
export const YooUI = (config: YooUIPluginConfig = {}): ReturnType<typeof plugin> => {
  const {
    themes: themeObject = {},
    defaultTheme = "light",
    layout: userLayout,
