...
export function extractNextUIImport(code: string): string[] {
  ...
  // Filter the lines to only include import statements from "@simao234430/react"
  const importStatements = lines.filter(
    (line) => line.startsWith("import") && line.includes("@simao234430/react"),
  );
...
export function updateTailwindConfig(tailwindConfig: string, componentNames: string[]): string {
  ...
  const lineIndex = lines.findIndex((line) =>
    line.includes("./node_modules/@simao234430/theme/dist/**/*.{js,ts,jsx,tsx}"),
  );
  ...
  if (lineIndex !== -1) {
    ...
    componentNames.forEach((componentName) => {
      ...
      lines.splice(
        lineIndex,
        0,
        `  "./node_modules/@simao234430/theme/dist/components/${name}.js",`,
      );
    });
  }
