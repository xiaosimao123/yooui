...
import {ChevronIcon} from "@simao234430/shared-icons";
import {CollectionBase, Expandable, MultipleSelection, Node, ItemProps} from "@react-types/shared";
import {BaseItem} from "@simao234430/aria-utils";
...
function TreeItem<T>(props: TreeItemProps<T>) {
  ...
  const renderFeaturebaseComponent = (key: string) => {
    ...
    if (key === "changelog")
      return (
        <YooUILink as={Link} className={cn} color="foreground" href="#">
          <FbChangelogButton />
        </YooUILink>
      );

    return (
      <YooUILink as={Link} className={cn} color="foreground" href="#" onClick={openFeedbackWidget}>
        <FbFeedbackButton />
      </YooUILink>
    );
  };
  ...
  const renderComponent = () => {
    ...
    return (
      <YooUILink as={Link} className={clsx(cn)} color="foreground" href={paths.pathname}>
        <span
          className={clsx(
            isSelected
