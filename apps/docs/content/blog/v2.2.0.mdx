...
The `YooUIProvider` component configures all NextUI components within it to navigate using the
...
- App Router
  ...
  export function Providers({children}: {children: React.ReactNode}) {
    ...
    return <YooUIProvider navigate={router.push}>{children}</YooUIProvider>;
  }
  ```
