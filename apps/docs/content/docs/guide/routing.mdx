...
frameworks use client side routers to avoid a full page reload when navigating between pages. The `YooUIProvider`
...
The `YooUIProvider` accepts a prop called `navigate`. This should be set to a function received from your
...
function App() {
  ...
  // 2. Add the `navigate` function to the `YooUIProvider`
  return (
    <YooUIProvider navigate={navigate}>
      <YourApplication />
    </YooUIProvider>
  );
}
...
export function Providers({children}: { children: React.ReactNode }) {
  ...
  return (
    <YooUIProvider navigate={router.push}>
      {children}
    </YooUIProvider>
  )
}
...
Now, Go to your `root` layout page and wrap it with the `YooUIProvider`:
...
> **Note**: Skip this step if you already set up the `YooUIProvider` in your app.
...
`navigate` function to the `YooUIProvider`:
...
Ensure that the component that calls `useNavigate` and renders `YooUIProvider` is inside the router 
component (e.g. `BrowserRouter`) so that it has access to React Router's internal context. The React Router `<Routes>` 
element should also be defined inside `YooUIProvider` so that links inside the rendered routes have access
to the router.
...
apps. `YooUIProvider` should be rendered at the `root` of each page that includes NextUI components, or in 
...
Now that you have set up the `YooUIProvider` in your app, you can use the `href` prop in the `Tabs`, 
...
`YooUIProvider` to navigate between pages.
